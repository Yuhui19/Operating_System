  ...
  for(int i = 0; i < ret.size(); ++i){
	...
	for(long j = first + 1; j < last; ++j){
	  if(tokens[j] == ">" || tokens[j] == "<" ){
		 ...
		 */
//		assert(false);
          char const* filename = tokens[j+1].c_str();
          if(tokens[j]==">"){
              ret[i].fdStdout=open(filename, O_CREAT | O_WRONLY, 0666);
          }else{
              ret[i].fdStdin = open(filename, O_RDONLY);
          }
          
          if (ret[i].fdStdout == -1 || ret[i].fdStdin == -1) {
              std::string err_msg = "Command: " + ret[i].exec + " failed to open file.\n";
...
//	assert(false);
      ...
      for (int i = 0; i < ret.size(); i++) {
          Command command = ret.at(i);
          if (command.fdStdin != 0) {
              if (close(command.fdStdin) == -1) {
